...
export class ClicksModule extends Module {
  ...
  close() {
    ...
    crossBtn.addEventListener('click', (event) => {
      ...
      //const closeId = event.path[1].id;
      this.removeModal(); //(closeId);closeByCross('.timer-modal-item');
    });
  }
  ...
  removeModal() {
    //removeModal(closeId)
    //this.startTime = -1;
    const modal = document.querySelector('.timer-modal-item'); //(`#${closeId}`)
